...
export default function GraphCorruptionPanel({
  ...
  const labels = lang === 'en'
    ? {
        title: 'Graph state integrity',
        dangling: 'Broken edges in document',
        stale: 'Removed on last import (audit)',
        nodes: 'Nodes in GraphDocument',
        edges: 'Edges (valid / total)',
        ghostSel: 'Ghost selection ids',
        orphans: 'Disconnected node ids',
        purge: 'Remove broken edges',
        reset: 'Fully reset corrupted graph',
        jump: 'Highlight on canvas',
        none: 'No corruption detected',
      }
    : {
        title: 'Целостность graph state',
        dangling: 'Битые рёбра в документе',
        stale: 'Удалены при загрузке (аудит)',
        nodes: 'Блоков в GraphDocument',
        edges: 'Рёбер (валидных / всего)',
        ghostSel: 'Ghost selection ids',
        orphans: 'Отключённые node ids',
        purge: 'Удалить битые связи',
        reset: 'Полностью сбросить повреждённый graph',
        jump: 'Подсветить на холсте',
        none: 'Повреждений не обнаружено',
      };
